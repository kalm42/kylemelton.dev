...
const IndexPage = () => {
  const [currentPos, setCurrentPos] = useState({ id: "", ratio: 0 })
  ...
  const scrollPos = {
    aboutme: { ratio: 0, id: "aboutme" },
    contactme: { ratio: 0, id: "contactme" },
    resume: { ratio: 0, id: "resume" },
    skills: { ratio: 0, id: "skills" },
    projects: { ratio: 0, id: "projects" },
  }

  const watch = function(entries) {
    console.log(entries)
    entries.map(entry => {
      scrollPos[entry.target.id].ratio = entry.intersectionRatio
      return entry
    })
    const active = Object.values(scrollPos).reduce(
      (acc, val) => (val.ratio ? val : acc),
      currentPos
    )
    console.log("Active: ", active)
    if (active.ratio > currentPos.ratio) {
  ...
  useEffect(() => {
    ...
    observer.observe(contactMe.current)
    observer.observe(resume.current)
      ...
      <div ref={rootRef}>
        ...
        <div ref={aboutMe} id="aboutme">
          ...
          <p>
            ...
            learning backend work as well. I’m relatively new to this and love
            working in teams so that I can learn new techniques for solving the
            everyday problems that arise. The best part of this career is
            getting to make stuff that works.
          </p>
          {/* Profile Image */}
          <p>
        ...
        <div ref={resume} id="resume">
          ...
          <p>
            ...
            requests for code review by senior developers. Working in a monorepo
            deploying an electron app, and a web app. The codebase is mostly
            React Typescript using both high order components and render props.
          </p>
          <h3>Kimmel &amp; Associates</h3>
          <p>November 2019</p>
